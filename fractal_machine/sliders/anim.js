(function ($) {
	window.onload = function(e) {
		...
		init = function() {
			...
			$.id("settings_on").addEventListener('click', function() {
				settings_on = !settings_on;
				resize();
			});
			$.id("flip").addEventListener('click', function() {
				flipped = !flipped;
				drawCanvas();
			});
		}();
		...
		function reset() {
			...
			$.set('content', {width:'100%', height:'100%', overflow:'hidden'});
		...
		function resize(){
			...
			fractal.height = H;
			drawCanvas();
			$.set('settings', {display: settings_on ? 'block' : 'none' })
		};

		function drawCanvas() {
			//
			$.set('fractal', {display: flipped ? 'none' : 'block'});
			//
		...
		function drawBranch(rota) {
			fractal_ctx.resetTransform();
			fractal_ctx.fillStyle = shape_color;
